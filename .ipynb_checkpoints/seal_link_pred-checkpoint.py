...
def test():
    ...
    return pos_val_pred, neg_val_pred, pos_test_pred, neg_test_pred, results
...
for run in range(args.runs):
    ...
    for epoch in range(start_epoch, start_epoch + args.epochs):
        ...
        if epoch % args.eval_steps == 0:
            pos_val_pred, neg_val_pred, pos_test_pred, neg_test_pred, results = test()
            
            # saving predictions
            
            pos_val = torch.cat((pos_val_edge, pos_val_pred.unsqueeze(1)), dim=1)
            neg_val = torch.cat((neg_val_edge, neg_val_pred.unsqueeze(1)), dim=1)
            pos_test = torch.cat((pos_test_edge, pos_test_pred.unsqueeze(1)), dim=1)
            neg_test = torch.cat((neg_test_edge, neg_test_pred.unsqueeze(1)), dim=1)
            
            val_pred = torch.cat((pos_val, neg_val))
            test_pred = torch.cat((pos_test, neg_test))
            
            val_save_path = os.path.join(args.res_dir, 'run{}_model_checkpoint{}_val_pred.pt'.format(run+1, epoch))
            torch.save(val_pred, val_save_path)
            
            test_save_path = os.path.join(args.res_dir, 'run{}_model_checkpoint{}_test_pred.pt'.format(run+1, epoch))
            torch.save(test_pred, test_save_path)
    ...
    min_idx = all_losses.index(min(all_losses))
    min_model_txt = os.path.join(args.res_dir, 'min_loss_model.txt')
    f = open(min_model_txt, "a")
    f.write(args.run_time + " best performing epoch: " + str(min_idx) + "\n")
    f.close()
